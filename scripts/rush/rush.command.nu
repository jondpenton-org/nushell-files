...
export def rush-add [
  --all (-a) # If specified, the dependency will be added to all projects.
  --caret (-c) # If specified, the SemVer specifier added to the package.json will be a prepended with a "caret" specifier ("^").
  --dev (-d) # If specified, the package will be added to the "devDependencies" section of the package.json
  --exact (-e) # If specified, the SemVer specifier added to the package.json will be an exact version (e.g. without tilde or caret).
  --make-consistent (-m) # If specified, other packages with this dependency will have their package.json files updated to use the same version of the dependency.
  --skip-update (-s) # If specified, the "rush update" command will not be run after updating the package.json files.
...
export def rush-install [
  ...
  --purge (-p) # Perform "rush purge" before starting the installation
] {
  let flags = (
    build-flags {
