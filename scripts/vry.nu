...
export def vry-match-elo [] {
  let match_table = $in | from vry | where name != ''
  let ranks = [
    `Iron`,
    `Bronze`,
    `Silver`,
  ...
  let teams_num = ($match_table | length) / 5
    ...
    | each { |team_num|
        let team_ranks_total_rr = (
          $match_table
            ...
            | each { |it|
                let peak_rank = $it.peak_rank | str replace `\s\(e\da\d\)` ''
                let rank_to_parse = (
                ...
                if $rank_to_parse in [`Iron 1`, `Unranked`] {
                  ...
                  let rank_index = (
                    $ranks
                      | enumerate
                      | std iter find { $in.item == $rank }
                      | $in.index
                  )
                  let rank_base_rr = $rank_index * 300
                  let tier = $rank_parts.1 | into int

                  $rank_base_rr + (($tier - 1) * 100) + $rr
                }
              }
        )
        let average_rr = $team_ranks_total_rr | math avg
        let rank_division_result = $average_rr / 300
        let rank_index = $rank_division_result | math floor
        let rank = $ranks | get $rank_index
        let tier_division_result = (
          ($rank_division_result - $rank_index) * 300 / 100
        )
        let tier = ($tier_division_result | math floor) + 1
        let rr = ($tier_division_result - ($tier - 1)) * 100
...
export def "from vry" [] {
  ...
  let table = (
    $table | each { |it|
      [rr, hs, level] | reduce --fold $it { |key, row|
        $row | update $key {
          if not ($in | is-empty) {
            into int
          }
